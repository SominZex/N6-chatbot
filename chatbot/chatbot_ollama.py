...
- For calculating median, explain that MySQL requires custom logic such as ordering by totalProductPrice, LIMIT + OFFSET.
...
SELECT 
    CAST(
        SUBSTRING_INDEX(
            SUBSTRING_INDEX(
                GROUP_CONCAT(total_sales ORDER BY total_sales), 
                ',', 
                CEIL(COUNT(*)/2)
            ), 
            ',', 
...
- For **variance** and **stddev**:
  SELECT storeName, VAR_SAMP(totalProductPrice) AS variance_sales, STDDEV_SAMP(totalProductPrice) AS stddev_sales FROM sales_data GROUP BY storeName;
